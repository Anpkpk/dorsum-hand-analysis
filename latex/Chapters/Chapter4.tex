...
\begin{table}[h!]
\caption{So sánh thời gian thực thi giữa CPU và GPU (đơn vị: ms)}
\label{tab:cpu_vs_gpu_performance}
\centering
\footnotesize % Thu nhỏ chữ xuống cỡ footnote để tiết kiệm diện tích
\setlength{\tabcolsep}{6pt} % Thu hẹp khoảng cách giữa các cột vừa phải
\begin{tabular}{l c c c}
    \toprule
    \textbf{Tác vụ / Kernel} & \textbf{CPU (ms)} & \textbf{GPU (ms)} & \textbf{Tăng tốc (Speed-up)} \\
    \midrule
    Ép khối 3D xuống 2D (MIP) & $12,450.2$ & $185.6$ & $67.08\times$ \\
    Bộ lọc cửa sổ trượt (Blur) & $4,120.5$ & $54.2$ & $76.02\times$ \\
    Phân ngưỡng nhị phân & $850.3$ & $12.4$ & $68.57\times$ \\
    Khởi tạo ma trận GLCM & $18,940.8$ & $245.1$ & $77.28\times$ \\
    \midrule
    \textbf{Tổng thời gian} & $\mathbf{36,361.8}$ & $\mathbf{497.3}$ & $\mathbf{73.12\times}$ \\
    \bottomrule
\end{tabular}
\end{table}
